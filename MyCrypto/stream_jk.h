...
public:
    LFSR(const QString& seed) : lfsr_(seed.toStdString()), key_(seed) {}
        void shift()
        {
            ...
            bool feedback = lfsr_[0] ^ (key_[0]=='1');
...
// JK触发器类
class MYCRYPTO_EXPORT JK_FlipFlop {
public:
...
QString MYCRYPTO_EXPORT work_init(QString plaintext, QString key);
// 流密码加密函数
template <int size1, int size2>
QString MYCRYPTO_EXPORT encrypt(const QString& plaintext, LFSR<size1>& lfsr1, LFSR<size2>& lfsr2, JK_FlipFlop& jk);
// 流密码解密函数与加密过程相同
template <int size1, int size2>
QString MYCRYPTO_EXPORT decrypt(const QString& ciphertext, LFSR<size1>& lfsr1, LFSR<size2>& lfsr2, JK_FlipFlop& jk);
